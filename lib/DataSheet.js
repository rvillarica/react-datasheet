  ...
  _createClass(DataSheet, [{
    ...
    value: function onMouseOver(i, j) {
      ...
      if (this.state.dragging) {
        ...
        let offsetY = i - this.state.dragStart.i;
        let offsetX = j - this.state.dragStart.j;
        //shift selected cells, and shift selection
        console.log("offset: " + offsetY + ", " + offsetX);
        let newStart = {i: this.state.dragTgtStart.i + offsetY,
                        j: this.state.dragTgtStart.j + offsetX};
        let newEnd = {i: this.state.dragTgtEnd.i + offsetY,
                      j: this.state.dragTgtEnd.j + offsetX};
    ...
    value: function setDragging(row,col) {
      this._setState({dragStart: {i: row, j: col}, 
                      dragTgtStart: this.state.start,
                      dragTgtEnd: this.state.end,
                      dragging: true});
    }
  }, {
    key: 'onMouseUp',
