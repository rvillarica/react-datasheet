...
var DataSheet = function (_PureComponent) {
  ...
  _createClass(DataSheet, [{
    ...
    value: function transferDraggedCells(offsetI, offsetJ) {
      ...
      for (let i = Math.min(selStart.i,selEnd.i); i <= Math.max(selStart.i,selEnd.i); i++) {
        for (let j = Math.min(selStart.j,selEnd.j); j <= Math.max(selStart.j,selEnd.j); j++) {
      ...
      if (this.props.onCellsChanged) {
        this.props.onCellsChanged(cells);
        this.onRevert();
      }
    }
  }, {
